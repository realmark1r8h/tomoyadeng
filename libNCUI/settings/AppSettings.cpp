...
namespace amo {
    ...
    void AppSettings::InitDefaultCefSettings() {
        ...
        CEFSTRING_DEFAULT_ARGS_SETTINGS(locale, "zh-CN");						//!< 语言环境默认中文
        ...
        CEFSTRING_DEFAULT_ARGS_SETTINGS(resources_dir_path, strResourcesPath);	//!< 设置资源目录
        CEFSTRING_DEFAULT_ARGS_SETTINGS(locales_dir_path, strLocalesPath);		//!< local目录
        DEFAULT_ARGS_SETTINGS(pack_loading_disabled, false);					//!< .
        DEFAULT_ARGS_SETTINGS(remote_debugging_port, 8088);						//!< 调试端口
        DEFAULT_ARGS_SETTINGS(uncaught_exception_stack_size, 1000);				//!< 不知道该设置多少
        DEFAULT_ARGS_SETTINGS(context_safety_implementation, true);				//!< .
        DEFAULT_ARGS_SETTINGS(ignore_certificate_errors, true);					//!< 忽略证书错误
    ...
    bool AppSettings::UpdateCefAppSettings() {
        ...
        BOOL_ARGS_SETTING(multi_threaded_message_loop);							//!< 禁止多线程消息循环，会出问题
        ...
        INT_ARGS_SETTING(uncaught_exception_stack_size);						//!< 不知道该设置多少
