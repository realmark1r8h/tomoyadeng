...
void compressOut(CMyComPtr< IOutArchive > outArc, CMyComPtr< T > outStream,
    ...
    HRESULT result = outArc->UpdateItems(outStream,
                                         static_cast< UInt32 >(in_items.size()), updateCallback);
...
BitCompressor::BitCompressor(const Bit7zLibrary& lib,
...
void BitCompressor::compress(const vector< wstring >& in_paths,
...
void BitCompressor::compressFile(const wstring& in_file,
                                 const wstring& out_archive) const {
...
void bit7z::BitCompressor::compress(const std::vector<FSItem>& in_files,
                                    const wstring& out_archive) const {
    compressToFileSystem(in_files, out_archive);
}


void BitCompressor::compressDirectory(const wstring& in_dir,
                                      const wstring& out_archive, bool recursive) const {
    compressFiles(in_dir, out_archive, L"*", recursive);
}

void BitCompressor::compressFiles(const wstring& in_dir,
                                  const wstring& out_archive, const wstring& filter,
...
void BitCompressor::compressFile(const wstring& in_file,
                                 vector< byte_t >& out_buffer) const {
    FSItem item(in_file);
...
void BitCompressor::compressToFileSystem(const vector< FSItem >& in_items,
        const wstring& out_archive) const {
        
        
    compressOut(outArchive, outFileStream, in_items, *this);
}

// FS -> Memory
void BitCompressor::compressToMemory(const vector< FSItem >& in_items,
                                     vector< byte_t >& out_buffer) const {
    if (in_items.size() == 0) {
    ...
    CMyComPtr< IOutArchive > outArc = initOutArchive(mLibrary, mFormat,
                                      mCompressionLevel, mCryptHeaders, mSolidMode);
...
bool bit7z::BitCompressor::open(const std::wstring& out_archive) {
    outArchive = initOutArchive(mLibrary, mFormat,
                                mCompressionLevel, mCryptHeaders, mSolidMode);
                                
                                
                                
    if (mVolumeSize > 0) {
        COutMultiVolStream* outMultiVolStreamSpec = new COutMultiVolStream(mVolumeSize,
                out_archive);
        ...
        * have any problem... until you try to compress files with GZip format! In that case your program will crash!! */
        ...
        if (!outFileStreamSpec->Open(out_archive.c_str(), OPEN_ALWAYS)) {
...
bool bit7z::BitCompressor::close() {

    if (outFileStream) {
        outFileStream->Release();
        outFileStream.Detach();
    }
