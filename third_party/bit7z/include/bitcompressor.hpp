...
#include "bit7z/lib/7zSDK/CPP/7zip/Archive/IArchive.h"
#include "bit7z/lib/7zSDK/CPP/7zip/IStream.h"
...
namespace bit7z {
    ...
    public:
        ...
        bool open(const std::wstring& out_archive);
         ...
         */
        void compress(const vector< wstring >& in_paths,
         ...
         * @param out_archive   the path (relative or absolute) to the output archive file.
         */
        void compressFiles(const vector< wstring >& in_files,
                           const wstring& out_archive) const;
                           
        void compress(const std::vector<filesystem::FSItem>& in_files,
                      const wstring& out_archive) const;
    ...
    private:
        void compressToFileSystem(const vector< FSItem >& in_items,
                                  const wstring& out_archive) const;
        void compressToMemory(const vector< FSItem >& in_items,
