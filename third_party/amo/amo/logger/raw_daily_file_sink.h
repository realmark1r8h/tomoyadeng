...
namespace spdlog {
    namespace sinks {
        ...
        */
        struct month_dateonly_daily_file_name_calculator {
            ...
            static filename_t calc_filename(const filename_t& basename, const filename_t& extension) {
                std::tm tm = spdlog::details::os::localtime();
                std::conditional<std::is_same<filename_t::value_type, char>::value, fmt::MemoryWriter, fmt::WMemoryWriter>::type w;
                w.write(SPDLOG_FILENAME_T("{:04d}{:02d}\\{}_{:04d}-{:02d}-{:02d}.{}"), tm.tm_year + 1900, tm.tm_mon + 1, basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, extension);
                return w.str();
            }
        };
        ...
        */
        ...
        typedef raw_daily_file_sink<std::mutex, month_dateonly_daily_file_name_calculator> raw_daily_file_sink_mt;
        typedef raw_daily_file_sink<details::null_mutex, month_dateonly_daily_file_name_calculator> raw_daily_file_sink_st;
    }
}
