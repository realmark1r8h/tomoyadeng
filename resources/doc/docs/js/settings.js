...
$(function() {
	...
	for(var p in defaultFileSettings) {
		var val = fileKeyMaps[p];
		if(val) {
			fillElement2($('#fileSettings .with-padding'), p, defaultFileSettings[p], val);
		}
	...
	for(var p in defaultAppSettings) {
		var val = appKeysMaps[p];
		if(val) {
			fillElement2($('#appSettings .with-padding'), p, defaultAppSettings[p], val);
		}
	...
	for(var p in defaultSplashSettings) {
		var val = splashKeysMaps[p];
		if(val) {
			fillElement2($('#splashSettings .with-padding'), p, defaultSplashSettings[p], val);
		}

		var str = defaultSplashSettings[p] + ',';
		console.dir(p + "," + str);
	}
	...
	$('#commit').click(function() {
		...
		var e = $('#splashSettings').find('input');
		var a1 = {};
		var b1 = {};
		var c1 = {};
		var d1 = {};
		...
		var splashSettings = $.extend({}, defaultSplashSettings, e1);
		console.dir(fileSettings);
		console.dir(appSettings);
		console.dir(browserSettings);
		console.dir(splashSettings);

		rcedit.commit(JSON.stringify(fileSettings),
			JSON.stringify(appSettings),
			JSON.stringify(browserSettings),
			JSON.stringify(splashSettings));
		var dlg = (new $.zui.ModalTrigger({
			size: 'center',
			custom: function() {
				return $('#templateAfterCommit').html();
			}
		}));
		dlg.show();
		...
		$('#openFolder').on('click', function(){
			include('shell');
			shell.showItemInFolder(fileSettings.OriginalFileName);
			dlg.close();
		});
	});
